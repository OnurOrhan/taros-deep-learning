...
def main():
    ...
    for medium in audio_mediums:
        ...
        dot =  AudioSegment.from_ogg(path_get + medium + "-dot.ogg")
        dash = AudioSegment.from_ogg(path_get + medium + '-dash.ogg')
        for character, code in morse_code.items():
            ...
            character_audio.export(path_store + medium + "/" + character + ".ogg", format="ogg")
